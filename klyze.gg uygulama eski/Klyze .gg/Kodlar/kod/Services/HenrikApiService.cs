...
namespace ValorantAutoClicker.Services
{
    ...
    public class HenrikApiService : IDisposable
    {
            ...
            string name, string tag, CancellationToken ct = default)
        {
            ...
            // 3. Rütbeyi Türkçeye çevir
            var rutbe = MapRutbe(mmr?.CurrentTierPatched ?? "");
        ...
        private static string MapRutbe(string tierPatched)
        {
            if (string.IsNullOrEmpty(tierPatched)) return "";
            var lower = tierPatched.ToLowerInvariant();

            if (lower.Contains("radiant"))                    return "Radyant";
            if (lower.Contains("immortal"))                   return "Ölümsüz";
            if (lower.Contains("ascendant"))                  return "Yükselen";
            if (lower.Contains("diamond"))                    return "Elmas";
            if (lower.Contains("platinum"))                   return "Platin";
            if (lower.Contains("gold"))                       return "Altın";
            if (lower.Contains("silver"))                     return "Gümüş";
            if (lower.Contains("bronze"))                     return "Bronz";
            if (lower.Contains("iron"))                       return "Demir";
            if (lower.Contains("unrated") || lower == "")    return "";

            return tierPatched; // bilinmeyen → olduğu gibi döndür
        }

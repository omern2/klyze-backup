...
enum DetectionFlags //used in client-server comms to flag cheaters. needs to be visible to NetClient thus not inside the Detections class since we don't want to #include Detections.hpp from NetClient.hpp
{
    ...
    // GÜÇLÜ TESPIT BAYRAKLARI ----------------
    HIDDEN_PROCESS = 20000,       // NtQuery vs WinAPI farkı (gizlenmiş process)
    NTDLL_HOOK,                   // ntdll syscall stub'ı hook'lanmış
    HIDDEN_MODULE,                // Yüklü modüller listesinde olmayan PE/çalıştırılabilir bellek
    HARDWARE_BREAKPOINT,          // DR0-DR3 register'larında breakpoint var
    TIMING_ANOMALY,               // Execution süresi anormal (debugger/VM)
    // GÜÇLÜ TESPIT BAYRAKLARI ----------------
};
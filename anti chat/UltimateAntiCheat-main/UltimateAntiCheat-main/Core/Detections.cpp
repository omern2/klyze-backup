...
void Detections::Monitor(__in LPVOID thisPtr)
{
    ...
    while (Monitoring)
    {
        ...
        // Teknik 1: Gizlenmiş process tespiti (NtQuery vs WinAPI çapraz kontrol)
        if (StrongDetections::DetectHiddenProcesses())
        {
            Logger::logf(Detection, "Hidden process detected via cross-reference check!");
            Monitor->EvidenceManager->AddFlagged(DetectionFlags::HIDDEN_PROCESS);
        }

        // Teknik 2: NTDLL syscall hook tespiti
        if (StrongDetections::DetectNtdllHooks())
        {
            Logger::logf(Detection, "NTDLL syscall hook detected!");
            Monitor->EvidenceManager->AddFlagged(DetectionFlags::NTDLL_HOOK);
        }

        // Teknik 3: Gizli modül / manuel map tespiti (VAD scan)
        {
            int hiddenModCount = StrongDetections::ScanForHiddenModules();
            if (hiddenModCount > 0)
            {
                Logger::logf(Detection, "Hidden/suspicious executable memory region(s) found: %d", hiddenModCount);
                Monitor->EvidenceManager->AddFlagged(DetectionFlags::HIDDEN_MODULE);
            }
        }

        // Teknik 4: Hardware breakpoint tespiti (DR register kontrolü)
        if (StrongDetections::DetectHardwareBreakpoints())
        {
            Logger::logf(Detection, "Hardware breakpoints detected in thread context!");
            Monitor->EvidenceManager->AddFlagged(DetectionFlags::HARDWARE_BREAKPOINT);
        }

        // Teknik 5: Timing anomali tespiti
        if (StrongDetections::DetectTimingAnomaly())
        {
            Logger::logf(Detection, "Timing anomaly detected, possible debugger or VM slowdown!");
            Monitor->EvidenceManager->AddFlagged(DetectionFlags::TIMING_ANOMALY);
        }

        // ---- GÜÇLÜ TESPIT BLOĞU SONU ----

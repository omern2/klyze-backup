...
int main(int argc, char** argv)
{
    ...
    const std::string logFileName = std::string(exeDir.begin(), exeDir.end()) + "UltimateAnticheat.log";
    ...
    explanations = 
    {
        ...
        { DetectionFlags::HIDDEN_PROCESS, "Hidden process detected via NtQuery vs WinAPI cross-reference" },
        { DetectionFlags::NTDLL_HOOK, "NTDLL syscall stub was hooked (first byte anomaly)" },
        { DetectionFlags::HIDDEN_MODULE, "Hidden/manually mapped executable memory region found" },
        { DetectionFlags::HARDWARE_BREAKPOINT, "Hardware breakpoints found in thread DR registers" },
        { DetectionFlags::TIMING_ANOMALY, "Execution timing anomaly detected (debugger/VM slowdown)" },
    };

...
namespace ValorantAutoClicker.ViewModels
{
    public partial class AgentViewModel : ObservableObject
    {
        ...
        public void StartWithCps(int cps)
        {
            if (Positions.Count == 0) { _statusCallback?.Invoke("Önce pozisyon ekleyin!"); return; }
            if (_clickingService.IsRunning) { _clickingService.Stop(); _statusCallback?.Invoke("Durduruldu"); return; }
            var ms = cps > 0 ? 1000 / cps : 10;
            if (ms < 1) ms = 1;
            _statusCallback?.Invoke($"Başlatılıyor: {cps} CPS = {ms}ms");
            if (int.TryParse(Duration, out var sec) && sec > 0)
                _ = _clickingService.StartAsync(Positions, ms, sec);
            else
                _ = _clickingService.StartAsync(Positions, ms);
        }
        ...
        private async Task ToggleClicking()
        {
            ...
            else
            {
                ...
                if (int.TryParse(Duration, out var sec) && sec > 0)
                    await _clickingService.StartAsync(Positions, Speed, sec);
                else

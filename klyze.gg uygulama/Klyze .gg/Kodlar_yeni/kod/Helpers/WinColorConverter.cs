...
namespace ValorantAutoClicker.Helpers
{
    ...
    {
        private static readonly SolidColorBrush Yesil = new(Color.FromRgb(0x22, 0xC5, 0x5E));
        private static readonly SolidColorBrush Kirmizi = new(Color.FromRgb(0xEF, 0x44, 0x44));
    ...
    public class EloRenkConverter : IValueConverter
    {
        private static readonly SolidColorBrush Gri     = new(Color.FromRgb(0x55, 0x55, 0x55));
        private static readonly SolidColorBrush Kirmizi = new(Color.FromRgb(0xFF, 0x46, 0x55));
        private static readonly SolidColorBrush Turuncu = new(Color.FromRgb(0xFF, 0x8C, 0x00));
        private static readonly SolidColorBrush Sari    = new(Color.FromRgb(0xFF, 0xD7, 0x00));
        private static readonly SolidColorBrush Yesil   = new(Color.FromRgb(0x00, 0xD2, 0x6A));
        ...
        public object Convert(object value, Type targetType, object parameter, CultureInfo culture)
        {
            ...
            {
                if (elo <= 0) return Gri;
                if (elo < 800) return Kirmizi;
                if (elo < 1300) return Turuncu;
                if (elo < 2000) return Sari;
                return Yesil;
            }
            return Gri;
        }
